...
/***************************************************************************//**
 * @brief
 *	Function to initialize the LEUART for BLE operation
 *
 * @details
 * 	Initializes a struct with all information needed to open the LEUART for BLE
 *
 * @note
 * 	Called once in the beginning from app_peripheral_setup
 *
 * @param[in] tx_event
 *   The TX done event
 *
...
void ble_open(uint32_t tx_event, uint32_t rx_event){
	...
	open.rx_en = true;
	open.tx_en = true;
	open.rx_pin_en = LEUART_ROUTEPEN_RXPEN;
	open.tx_pin_en = LEUART_ROUTEPEN_TXPEN;

	leuart_open(LEUART0, &open);
...
/***************************************************************************//**
 * @brief
 * 	Function to write a string to the BLE
 *
 * @details
 *  Calls leuart_start with the string passed in
 *
 * @note
 *  Called from humidity_done_cb which is called every time a humidity value is read on LETIMER underflow
 *
 * @param[in] *string
...
void ble_write(char* string){
	...
	leuart_start(LEUART0, string, str_len);
}
...
bool ble_test(char *mod_name){
	...
	// ANSWER: in Polling, CPU constantly checking for events, in interrupts, CPU is alerted for an event
	// How does interrupts benefit the system for low energy operation?
	// ANSWER: The CPU does not need to be always working
	// How does interrupts benefit the system that has multiple tasks?
	// ANSWER: Separate interrupt handler for each task
	...
	if (status & LEUART_STATUS_RXENS) {
		...
		while (!(leuart_status(HM10_LEUART0) & LEUART_STATUS_RXENS)); //STUCK HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	}
...
//	leuart_cmd_write(HM10_LEUART0, (LEUART_CMD_CLEARRX | LEUART_CMD_CLEARTX));
	...
	// DSD HM10 module?
	// ANSWER: Need to make sure the connection has ended before the bluetooth module can receive the command
